...
@router.get("/ui")
def payments_ui(request: Request, db: Session = Depends(get_db)):
    payments = svc.list_payments(db)
    return templates.TemplateResponse("payments.html", {"request": request, "payments": payments})

@router.get("/ui/new")
def payment_new_ui(request: Request, db: Session = Depends(get_db)):
    contracts = db.query(Contract).order_by(Contract.id.desc()).all()
    return templates.TemplateResponse("payment_form.html", {"request": request, "contracts": contracts, "action": "create"})

@router.get("/ui/{payment_id}")
def payment_detail_ui(payment_id: int, request: Request, db: Session = Depends(get_db)):
